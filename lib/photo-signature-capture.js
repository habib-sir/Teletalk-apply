...
(function (global) {
  ...
  // ---------------------------------------------------------------
  function buildCropOverlay(container, img, aspectW, aspectH) {
    container.innerHTML = '';
  ...
  function showSavedThumbnail(container, base64, caption, onRemove) {
    container.innerHTML = '';
    const wrap = document.createElement('div');
  ...
  async function refreshPreviews(profileId) {
    ...
    if (photoPrev) {
      ...
      if (p && p.base64) {
        showSavedThumbnail(photoPrev, p.base64, `Photo: 300×300px (${p.kb} KB)`, async () => {
          await deleteProfilePhoto(profileId);
    ...
    if (sigPrev) {
      ...
      if (s && s.base64) {
        showSavedThumbnail(sigPrev, s.base64, `Signature: 300×80px (${s.kb} KB)`, async () => {
          await deleteProfileSignature(profileId);
